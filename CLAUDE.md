...
- `build_unified_market_state` takes 12 positional args — args 3/4 are `liquidity_walls: LiquidityWalls, liquidity_analytics: LiquidityAnalytics`; the last three are `liquidations: LiquidationState`, `global_oi: GlobalOIState`, `spot_price: Option<f64>`. All call sites (in `lib.rs` and `quant.rs` tests) must be updated together when the signature changes.
