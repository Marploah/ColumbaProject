...
cd backend && cargo test           # run all unit tests
cd backend && cargo test -- <name> # run single test by name (substring match)
```
...
- **`lib.rs`** — `pub async fn run()`: Axum server, kline seeding, all routes, background task spawning. Routes: `GET /api/snapshot`, `POST /api/analyze`, `GET /api/trades`, `GET /api/trades/summary`, `PATCH /api/trades/:id/outcome`, `POST /api/symbol`, `POST /api/interval`, `GET /ws` (WebSocket). Nine background tasks are spawned: eight receive a `watch::Receiver<String>` to react to symbol changes (`poll_open_interest` 15s, `poll_bybit_oi` 10s, `poll_okx_oi` 10s, `poll_spot_price` 10s, `poll_liquidity_walls` 10s, `poll_funding_rate` 30s, `poll_tf_biases` 5min, `stream_liquidations` persistent WS), plus `poll_fear_greed` (5min, no symbol dependency — global sentiment). CORS allowlist is hardcoded to `localhost:5173`, `127.0.0.1:5173`, `tauri://localhost`, `https://tauri.localhost` — adding new origins requires editing `run()` directly. Each `POST /api/analyze` spawns a `monitor_trade_outcome` Tokio task that watches the live price broadcast and auto-sets `TP_HIT`/`SL_HIT`/`EXPIRED` in the DB; the monitor times out after 24 hours.
- **`quant.rs`** — `UnifiedMarketState`, `CandleData`, `LiquidityWalls`, `TfBiases`, `ConfluenceMatrix`. CVD accumulation via `Decimal`, CVD slope (linear regression 20 candles), ATR-14 + RSI (`ta` crate), VWAP over candle window, funding settlement hours calculation, `compute_orderflow_state` (P5 advanced orderflow signals), `compute_volatility_regime` (P6 — ATR percentile + expansion direction). `build_unified_market_state(symbol, candles, walls, liquidity_analytics, tf_biases, oi_is_real, funding_rate, cache, cvd_seeded, liquidations, global_oi, spot_price)` assembles all signals — **every call site must pass all 12 args in order**. `SignalCache` caches ATR-14, RSI divergence, and `VolatilityState` keyed on candle count; recomputes only on candle close, never per-tick; invalidated on symbol/interval reset.
...
Domain structs held as fields on `UnifiedMarketState`; populated by `build_unified_market_state`. Most are serialized as nested JSON objects in the wire snapshot — the frontend receives them as-is. **Exception:** `liquidity_analytics` carries `#[serde(skip)]` and is never sent to the frontend; it is consumed only by `SignalInterpreter`.
...
- **`liquidity.rs`** — `WallInfo { price, size, persistence_polls, spoof_score, replenishment_detected }`, `LiquidityAnalytics { bid_wall, ask_wall, spoof_alert, feed_healthy }`, `WallTracker` (stateful tracker). `WallTracker` holds `HashMap<u64, WallEntry>` per side keyed on price quantized to centavos (`price * 100.0`). `update(bids, asks, now)` ingests raw `[[String;2]]` depth levels, runs 3×-mean significance filter, updates persistence/spoof/replenishment state, and refreshes `cached_walls`/`cached_analytics`. `mark_unhealthy()` on fetch errors, `reset()` on symbol change. Stored as `UnifiedMarketState.liquidity_analytics` with `#[serde(skip)]` — not on the wire. Wire-compatible `LiquidityWalls` still serialized separately. `AppState.wall_tracker` replaces the old `current_liquidity_walls` field. Spoof heuristic: `present ≤ 2 AND absent > 0` → score 0.80; `absent > present` → 0.55; `present ≥ 6` → 0.05.
...
- **`volatility.rs`** — `VolatilityRegime` enum (`Compression`/`Normal`/`Elevated`/`Extreme`/`Unknown`), `VolatilityState { regime, atr_percentile: Option<f64>, expanding: Option<bool>, regime_confidence: f32 }`. Regime is derived by `compute_volatility_regime` in `quant.rs`: samples ATR at up to 50 evenly-spaced historical endpoints (each using full candle history for stable Wilder EMA), then midpoint-ranks the current ATR to get its percentile. `expanding: Some(true)` = ATR trending up vs ~10% of history ago; `None` = stable (within ±5% threshold). Stored as `UnifiedMarketState.volatility`; cached in `SignalCache`.
- **`sentiment.rs`** — `FearGreedState { value: u8, classification, timestamp, feed_healthy }`. Polled every 5 min by `poll_fear_greed` in `lib.rs` from `https://api.alternative.me/fng/?limit=1` (data updates daily). No symbol dependency — global crypto sentiment. `feed_healthy=false` until first successful poll. Serialized to the wire (frontend receives it in snapshots).
- **`semantic.rs`** — `SemanticSignal { label, confidence, explanation, severity }`, `AiSemanticState` (one signal per domain: leverage, volatility, liquidity, orderflow, directional, liquidation, oi_divergence, basis, sentiment), `SignalInterpreter::interpret(&state) -> AiSemanticState`. Also runs `detect_contradictions` — cross-signal conflicts emitted as plain strings in the brief. `SignalInterpreter` is called only from `format_market_brief` in `ai.rs`; it is not stored on `UnifiedMarketState` to avoid a module cycle. `interpret_orderflow` uses a signal hierarchy: sweep overrides absorption overrides delta-momentum overrides base CVD/buy-pressure. `interpret_volatility` is regime-aware (uses `state.volatility.regime` + percentile + direction suffix). `interpret_liquidity` uses `state.liquidity_analytics`: spoof_alert → spoof warning takes priority; degraded feed → fallback to basic wall presence; healthy feed → persistence/replenishment labels + confidence scaling with `persistence_polls`. `interpret_sentiment` maps F&G value to Extreme Fear/Fear/Neutral/Greed/Extreme Greed with confidence 0.75–0.85. `detect_contradictions` includes: Extreme Greed + crowded longs → late-cycle euphoria warning; Extreme Fear + buying pressure → capitulation exhaustion signal.
...
- `build_unified_market_state` takes 13 positional args — args 3/4 are `liquidity_walls: LiquidityWalls, liquidity_analytics: LiquidityAnalytics`; the last four are `liquidations: LiquidationState`, `global_oi: GlobalOIState`, `spot_price: Option<f64>`, `sentiment: FearGreedState`. All call sites (in `lib.rs` and `quant.rs` tests) must be updated together when the signature changes.
- `AppState.wall_tracker: Arc<Mutex<WallTracker>>` replaced the old `current_liquidity_walls`. All reads use `t.snapshot_walls()` + `t.snapshot_analytics()`; symbol/interval resets call `reset()`; fetch errors call `mark_unhealthy()`. `WallTracker` is seeded at startup from the initial depth response so the first market state has real wall data.
...
- `UnifiedMarketState.volatility`: `regime=Unknown` and `atr_percentile=None` until at least 30 candles are available. `compute_volatility_regime` uses midpoint ranking (ties contribute 0.5) so uniform ATR history → 50th percentile → Normal, not Compression. Sampling at ≤50 points over the full buffer means the computation is O(n²/50) — acceptable at candle-close frequency but must not be called per-tick (enforced via `SignalCache`).
